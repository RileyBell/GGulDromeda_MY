...
<% 
	String request_sub = request.getParameter("board_sub"); 
	System.out.println("request_sub : " + request_sub);
%>
...
$(function(){
	...
	// board_sub를 request에 받아와서 sub_select select에 selected 하게 만든다
	
	var request_sub = "${requestScope.board_sub}";
	alert("리퀘스트의 서브 : " + request_sub);
	//$("#sub_select option:eq("+request_sub+")").attr("selected", "selected");
	
	$("#sub_select > option[value="+request_sub+"]").attr("selected", "selected");
	
});

	// select - 하위 게시판 선택했을때 작동
	function subSelect() {
	    var board_sub = document.getElementById("sub_select").value;
	    ...
	    location.href ="board_sub.bee?board_sub=" + board_sub + "&page_num=${requestScope.page_num}" + "&board_main=${requestScope.board_main}" + "&local_code=AA";
	}
	...
	function orderSelect() {
	    ...
	    var board_order = document.getElementById("order_select").value;
		 ...
		 // 지역코드는 지금 테스트를 위해 그냥 지정
	  	...
	  	alert("board_sub : " + board_sub);
	  	
	  	// board_sub가 null인 즉 하위게시판이 아예없는 게시판 > nothing으로 설정해주기
	
	    location.href ="board_order.bee?board_sub=" + board_sub + "&board_order=" + board_order +"&page_num=${requestScope.page_num}" + "&board_main=${requestScope.board_main}" + "&local_code=AA";
	}
				...
				<form class="form-horizontal" role="form">
					<div class="form-group" style="border:5px solid #F1F1F2; margin-bottom:0px">
						<div class="col-sm-5">
							...
							<!-- 분기문 -->
							<!-- TIP 게시판 -->
							<% if(board_main.equals("tip")) {%>
								<h1 style="color:#063a88;">TIP</h1>
							<% } %>
							<!-- 지역거래 게시판 -->
							<% if(board_main.equals("trade")) {%>
								<h1 style="color:#063a88;">지역거래</h1>
							<% } %>
							<!-- 광고 게시판 -->
							<% if(board_main.equals("ads")) {%>
								<h1 style="color:#063a88;">광고</h1>
							<% } %>
							<!-- 공지 & 이벤트 게시판 -->
							<% if(board_main.equals("notice")) {%>
								<h1 style="color:#063a88;">공지&이벤트</h1>
							<% } %>
							
						</div>
						
						<!-- 하위게시판이 있다면 하위게시판 select할 수 있도록! -->
						<!-- TIP 하위게시판 - 맛집, 지역, 코스 -->
						<% if(board_main.equals("tip")) {%>
							<div class="col-sm-2"></div>
							
							<div class="col-sm-3" align="right">
							<select class="form-control" id="sub_select" name="sub_select" onchange="subSelect()" style="width:auto;margin-top: 30px; margin-right:0" >
								<option value="nothing">하위 게시판</option>
						...
						<% if(board_main.equals("ads")) {%>
							...
							<div class="col-sm-3" align="right">
							<select class="form-control" id="sub_select" onchange="subSelect()" style="width:auto;margin-top: 30px; margin-right:0" >
								<option value="nothing">하위 게시판</option>
								<option value="meal">밥집</option>
								<option value="pub">술집</option>
								<option valeu="cafe">카페</option>
							</select>
						</div>
						<%} %>
						...
						<%-- 하위게시판이 없다면! --%>
						<% if(board_main.equals("trade")) { %>
							<div class="col-sm-5" id="sub_select" value="nothing"></div>
						<%} %>
						
						<!-- 정렬을 정하는 select -->
						<div class="col-sm-1" align="right" style="padding-left:0">
							<select class="form-control" id="order_select" onchange="orderSelect()" style="width:auto;margin-top: 30px; margin-left:0" >
								<option>정렬</option>
						...
						<c:forEach var="board_bean" items="${requestScope.board_list}">
							<div class="col-sm-3 col-md-3" style="border:5px solid #F1F1F2;">
								...
								<a href="http://google.com" class="thumbnail">	
										<!-- board_img_main 으로 이미지 띄우는 작업하기 -->			   
